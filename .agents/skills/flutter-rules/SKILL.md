---
name: flutter-rules
description: Flutter and Dart development rules, best practices, code style, state management, testing, and theming guidelines. Use when writing Flutter/Dart code, reviewing code, or making architectural decisions.
---
...
- **Fixes:** Use `dart fix --apply` to automatically fix many common errors,
  and to help code conform to configured analysis options.
- **Linting:** Use the Dart linter with a recommended set of rules to catch
  common issues. Use `flutter analyze` to run the linter.
...
- **Pub Tool:** To add or remove package dependencies from the project, use
  `flutter pub add <package_name>` and `flutter pub remove <package_name>`.
...
- **GoRouter Setup:** To use `go_router`, first add it to your `pubspec.yaml`
  using `flutter pub add go_router`.
...
- **Running Apps**: To run your app, use `flutter run` in terminal. Or use your
  IDE's Run features via the Flutter extension or plugin.
- **Use `flutter pub` for dependencies**: Do not manually modify dependencies in
  `pubspec.yaml`. Always use `flutter pub add` and `flutter pub remove`.
  - **Dev Dependencies**: Use `flutter pub add dev:<package_name>`.
  - Exception: It is OK to edit `pubspec.yaml` directly for description,
    version, or name changes.
- **Use Absolute Paths**: When specifying file paths, use absolute paths.
- After making Dart source code modifications, run `flutter analyze` before
  running tests and address static analysis issues first.
- Before committing changes to git, run `dart fix --apply` and `dart format .`.
