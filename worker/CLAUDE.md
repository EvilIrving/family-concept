...
- 对象存储：Cloudflare R2，当前 bucket 名 `kitchen-assets`，Worker 绑定名 `ASSETS`
...
- 新增或重构复杂逻辑时优先拆到 service 层；现有路由中仍有少量 multipart 解析、R2 写入与错误处理逻辑
...
POST   /dishes/:id/image_upload_url         — 获取 Worker 代理上传票据
PUT    /dishes/:id/image                    — Worker 代理上传图片到 R2
...
GET    /kitchens/:id/entitlement            — 获取厨房付费权益
POST   /kitchens/:id/iap/verify             — 验证 StoreKit 交易并同步权益
...
- `image_key TEXT` — R2 key，当前格式 `{kitchen_id}/{dish_id}.png`
...
- `created_at TEXT NOT NULL DEFAULT (datetime('now'))`
- `updated_at TEXT NOT NULL DEFAULT (datetime('now'))`
- 唯一索引：`original_transaction_id IS NOT NULL` 时唯一
- 活跃权益索引：按 `kitchen_id` 查询 `revoked_at IS NULL`
...
- 客户端本地完成图片裁切、标准化和 PNG 导出，只上传成品图
- `POST /dishes/:id/image_upload_url` 当前返回 Worker 代理上传路径、上传方法和 `image_key`
- `PUT /dishes/:id/image` 由 Worker 接收图片并写入 R2
- R2 key 当前格式：`{kitchen_id}/{dish_id}.png`
...
- 新增业务逻辑优先放入 service 层；修改现有路由时保持行为兼容，再逐步拆分过重处理函数
