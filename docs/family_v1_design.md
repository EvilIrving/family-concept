...
- 身份：Sign in with Apple，`apple_user_id`（sub）作为稳定用户主键
- 后端：Cloudflare Workers（API 层）+ D1（关系数据）+ R2（菜品图片）+ Durable Objects（订单实时广播）
- 图片路径：R2 bucket 名 `dishes`，key 格式 `{kitchen_id}/{dish_id}.jpg`
- 权限校验：Workers 中间件统一处理，D1 无 RLS 能力
- 不做账号密码、邮箱验证码、匿名访客、局域网模式

## 4. 身份与最小用户体系

- 首次进入 App 触发 Sign in with Apple（Apple 要求至少一次用户主动操作）
- Workers 用 Apple 公钥验证 identity token，取出 `sub`，查找或创建用户记录
- 后端签发自有 JWT 作 session，iOS 端存 Keychain，续期静默进行
- 用户表只存最小字段，不做找回、绑定其他登录方式、多设备合并等功能
...
用途：用户身份存储，跨 kitchen 共享。

字段：

- `id TEXT PRIMARY KEY` — UUID
- `apple_user_id TEXT NOT NULL UNIQUE` — Apple 返回的 sub
...
```
users ──< kitchen_members >── kitchens ──< dishes
...
1. 首次启动 → Sign in with Apple
2. Workers 查找或创建 `users` 记录，签发 session JWT
3. 若当前用户没有任何 `active` 的 `kitchen_members`，进入入驻页：
   - 主路径：输入邀请码加入已有 kitchen（输入框默认聚焦）
   - 次路径：下方小号文字按钮「创建我的私厨」，点击后展开输入 kitchen 名称
4. 加入或创建成功后进入主界面

### 6.2 邀请加入

- owner / admin 可查看和刷新邀请码，刷新后旧码立即失效
- 用户输入邀请码即自动加入为 `member`，v1 不做 owner 审批
...
所有接口都通过认证中间件校验 session JWT，注入 `current_user` 上下文。

```
POST   /auth/apple                          — 换取 session JWT
GET    /me                                  — 当前用户信息
...
DELETE /kitchens/:id/members/:user_id       — 踢人
...
- 实现 Apple 登录验证与 session JWT 签发
- 执行 `database.sql` 建表
- 实现核心 API（auth、kitchen、member、dish、order、order_items）
...
- 新建 SwiftUI 工程，集成 Sign in with Apple
- 实现入驻页（邀请码 / 创建私厨）
- 实现菜单页（查看 / 添加菜品）
- 实现点菜流程（追加到订单）
- 实现厨房视角（实时更新 item 状态）
