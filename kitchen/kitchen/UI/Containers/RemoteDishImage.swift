    ...
    var body: some View {
        Group {
            ...
            case .failure(let feedback, _):
                AppErrorPlaceholder(feedback: feedback, retryTitle: L10n.tr("Retry")) {
                    Task {
                        await loader.retry()
                    }
                }
    ...
    final class Loader: ObservableObject {
        ...
        func load(from url: URL) async {
            ...
            if let retainedValue {
                phase = .loading(LoadingContext(mode: .refresh, label: L10n.tr("Loading"), retainedValue: retainedValue))
            } else {
                phase = .loading(LoadingContext(mode: .initial, label: L10n.tr("Loading")))
            }
            ...
            do {
                ...
                #endif
                phase = .failure(.network(message: L10n.tr("Image load failed")), retainedValue: retainedValue)
            }
        }
