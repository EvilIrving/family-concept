...
本仓库内对 `kitchen/` 做功能开发或较大改动时，**必须先走计划、再写代码**，不允许跳过计划直接改源码。

1. 使用 **`/codex:rescue`**：根据任务描述写出计划（拆解步骤、文件与风险），不在这里落具体实现代码。
2. 使用 **`/codex:result`**：查看上一步计划的执行结果与产出，确认与预期一致后再进入实现。
3. **根据已确认的计划**再开始编写或修改 Swift 代码，实现须符合本文件其余规范。

**禁止**：在未使用上述流程形成并核对计划的情况下，直接编写或提交代码改动。
